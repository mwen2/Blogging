...
module.exports = {
	addPost: function(title, subject,tag, callback){
    	MongoClient.connect(url, function(err, database) {
        	...
        	myAwesomeDB.collection('post').insertOne( {
            	"title": title,
            	"subject": subject,
            	"tag": tag
        	},function(err, result){
	...
	updatePost: function(id, title, subject, callback){
		MongoClient.connect(url, function(err, database) {
			const myAwesomeDB = database.db('Blog');
		  	myAwesomeDB.collection('post').updateOne( 
	...
	getPost: function(callback){
		...
		MongoClient.connect(url, function(err, database){
			const myAwesomeDB = database.db('Blog');
			 myAwesomeDB.collection('post', function (err, collection) {
	...
	getTag: function(callback){
    	MongoClient.connect(url, function(err, database){
    		const myAwesomeDB = database.db('Blog');
         	myAwesomeDB.collection('tag', function (err, collection) {
            	collection.find().toArray(function (err, list) {
                	callback(list);
            	});
         	});
	...
	addTag: function(tagName, callback){
    	MongoClient.connect(url, function(err, database) {
    		const myAwesomeDB = database.db('Blog');
        	myAwesomeDB.collection('tag').insertOne( {
            	"name": tagName
        	},function(err, result){
            	assert.equal(err, null);
	...
	deletePost: function(id, callback){
		...
		MongoClient.connect(url, function(err, database){
			const myAwesomeDB = database.db('Blog');
			 myAwesomeDB.collection('post').deleteOne({
			 	_id: new mongodb.ObjectID(id)
			 },
			 function(err, result){
				assert.equal(err, null);
	...
	getPostWithId: function(id, callback){
		...
		MongoClient.connect(url, function(err, database){
			const myAwesomeDB = database.db('Blog');
			 myAwesomeDB.collection('post').findOne({
			 	_id: new mongodb.ObjectID(id)
			 },
			 function(err, result){
				assert.equal(err, null);
